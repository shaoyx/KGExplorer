...
@GraphAnalyticTool(
        name = "Interactive QBE Solver Http Ver",
        description = "The solver of interactively finding entities by given examples.The Entry of graph detection."
)
public class InteractiveQbeSolverHttp implements GenericGraphTool {
    private static final Logger logger = LogManager.getLogger(InteractiveQbeSolverHttp.class.getName());

    private IUserAgentHttp userAgent = new InteractiveUserAgentHttp();
//    private IKGExplorationSystem kgExplorationSystem = new KGExplorationSystem();
    private IKGExplorationSystemHttp kgExplorationSystem = new PriorKGExplorationSystemHttp();
